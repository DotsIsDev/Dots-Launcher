...
public sealed class MainWindow : Window
{
    ...
    {
        var surface=Surface(p,236,174,new(0,0,16,16));var gridbox=new Grid();
        var center=new Grid{Width=196,RowDefinitions=new("48,44,18"),RowSpacing=6,HorizontalAlignment=HorizontalAlignment.Center,VerticalAlignment=VerticalAlignment.Center};
        var image=new Image{Source=IconService.Get(p.Path,p.IconPath),Width=44,Height=44,Stretch=Stretch.Uniform,HorizontalAlignment=HorizontalAlignment.Center,VerticalAlignment=VerticalAlignment.Center};
        var title=new TextBlock{Text=p.Title,FontSize=16,FontWeight=FontWeight.SemiBold,TextAlignment=TextAlignment.Center,TextWrapping=TextWrapping.Wrap,HorizontalAlignment=HorizontalAlignment.Stretch,VerticalAlignment=VerticalAlignment.Center};
        var file=new TextBlock{Text=Available(p)?Path.GetFileName(p.Path):"FILE UNAVAILABLE",FontSize=11,Foreground=Muted,TextAlignment=TextAlignment.Center,TextTrimming=TextTrimming.CharacterEllipsis,HorizontalAlignment=HorizontalAlignment.Stretch,VerticalAlignment=VerticalAlignment.Center};
        center.Children.Add(image);center.Children.Add(title);Grid.SetRow(title,1);center.Children.Add(file);Grid.SetRow(file,2);gridbox.Children.Add(LaunchButton(center,p));gridbox.Children.Add(MenuButton(p));var check=Check(p);gridbox.Children.Add(check);Hover(surface,check);surface.Child=gridbox;return surface;
    }
